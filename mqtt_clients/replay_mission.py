...
def main(raw_args=None):
    ...
    try:
        with bosdyn.client.lease.LeaseKeepAlive(lease_client):
            ...
            #########################################################################################################################
            #Upload docking status to S3
            now = datetime.now()
            data = [0,now]

            with open('./dock.csv', 'w', encoding='UTF8', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(data)

            aws_bucket_name = 'element212-spot'  
            source_file = 'dock.csv'  
        ...
        with open('./dock.csv', 'a', encoding='UTF8', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(data)

        aws_bucket_name = 'spot-test-zoe'  
        source_file = 'dock.csv'  
        destination_file = source_file 
        upload_to_aws(aws_bucket_name, source_file, destination_file)
        ##########################################################################################################################
