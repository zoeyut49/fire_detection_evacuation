...
def upload_to_aws(bucket_name, source_file, destination_file):
    ...
    result = s3.Bucket(bucket_name).upload_file(source_file,destination_file)
...
def process_thread(args, request_queue, response_queue):
    ...
    while True:
        ...
        for i in range(boxes.shape[0]):
            ####################################################################
            if scores[i] < 0.45:      ## set threshold
        ...
        print('Found ' + str(num_objects) + ' person(s)')

##########################################################################################################
        if not args.no_debug and num_objects > 0:
##########################################################################################################
            ...
            now = datetime.now()
            data = [num_objects,score_array,now]

            with open('./detection.csv', 'w', encoding='UTF8', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(data)

            aws_bucket_name = 'element212-spot'  
            source_file_1 = 'detection.csv'  
            destination_file_1 = source_file_1  
            source_file_2 = 'network_compute_server_output.jpg' 
            destination_file_2 = str(now.strftime('%Y_%m_%d_%H_%M_%S')) + '.jpg'
            upload_to_aws(aws_bucket_name, source_file_1, destination_file_1)
            upload_to_aws(aws_bucket_name, source_file_2, destination_file_2)
            print('Upload human detection image to: "' + destination_file_2 + '"')
            #################################################################################################
